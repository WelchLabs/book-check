...
import { PROMPT_VERSION, ReviewSchema, SYSTEM_PROMPT, userPrompt, type ApiModel, type CliModel, type RawChunkReview } from "./review-spec"
import type { AiUsage, ChunkData, Finding } from "./types"

export type Provider = { kind: "cli"; model: CliModel; endpoint: string } | { kind: "api"; apiKey: string; model: ApiModel }
...
export async function runAiReview(
  ...
  const review = provider.kind === "cli" ? cliReviewer(provider.model, provider.endpoint) : await apiReviewer(provider.apiKey, provider.model)
